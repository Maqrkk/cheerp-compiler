...
void SIMDTransformPass::checkVectorCorrectness(Instruction& I)
{
	...
	if (vectorBitwidth != 128)
	{
		...
		{
			assert(isa<LoadInst>(I) || isa<BinaryOperator>(I) || isa<IntrinsicInst>(I) || isa<FPTruncInst>(I));
			for (User* U: I.users())
			{
				assert(isa<Instruction>(U));
				const Instruction* useI = cast<Instruction>(U);
				assert(isa<SExtInst>(useI) || isa<ZExtInst>(useI) || isa<TruncInst>(useI) || isa<FPExtInst>(useI) || isa<BinaryOperator>(useI) || isa<CmpInst>(useI) || isa<IntrinsicInst>(useI) || isa<ExtractElementInst>(useI) || isa<StoreInst>(useI));
			}
			return ;
		}
		...
		if (vectorBitwidth == 32 && (isa<TruncInst>(I) || isa<SelectInst>(I) || isa<PHINode>(I) || isa<BinaryOperator>(I)))
			return ;

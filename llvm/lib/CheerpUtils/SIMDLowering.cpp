...
struct SIMDLoweringVisitor: public InstVisitor<SIMDLoweringVisitor, VectorParts>
{
	...
	{
		if (!shouldLower(I.getResultElementType()))
			return VectorParts();

		const FixedVectorType* vecType = cast<FixedVectorType>(I.getResultElementType());
		const unsigned num = vecType->getNumElements();
		Type* elType = vecType->getElementType();
		IRBuilder<> Builder(I.getNextNode());
		VectorParts result;

		// We add first a bitcast to the base element type.
		// Then num - 1 GEPs for the remaining elements.
